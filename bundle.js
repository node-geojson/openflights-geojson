...
var App = React.createClass({
  ...
  onMapClickResults: function onMapClickResults(err, res) {
    if (!err && res.length) {
      this.setState({
        pendingFocus: res[0]
      });
  ...
  usePendingFocus: function usePendingFocus() {
    this.setState({
      focus: this.state.pendingFocus,
      pendingFocus: null
    });
  },
  discardPendingFocus: function discardPendingFocus() {
    this.setState({ pendingFocus: null });
  },
  discardFocus: function discardFocus() {
    this.setState({ focus: null });
  },
  render: function render() {
    if (this.map) {
      if (this.state.focus) {
        this.map.setFilter('routes-arc4', ['any', ['==', 'dst_id', this.state.focus.properties.id], ['==', 'src_id', this.state.focus.properties.id]]);
      } else {
    ...
    return React.createElement(
      ...
      this.state.focus && React.createElement(
        ...
        React.createElement(
          'a',
          { onClick: this.discardFocus, className: 'button' },
          'X'
        )
      ),
      this.state.pendingFocus && React.createElement(
        'div',
        { className: 'overlay' },
        React.createElement(
          'a',
          { onClick: this.usePendingFocus, className: 'button' },
          'Focus on ',
          this.state.pendingFocus.properties.name
        ),
        React.createElement(
          'a',
          { onClick: this.discardPendingFocus, className: 'button' },
          'X'
        )
      )
    );
